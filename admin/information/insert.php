...
if  (isset($_POST) && !empty($_POST)) {
    ...
    if (!empty($header)) {
        $sql_check = "SELECT * FROM db_info WHERE info_header = '$header'";
        $query_check = mysqli_query($conn, $sql_check);
        $row_check = mysqli_num_rows($query_check);
        if($row_check > 0) {
            ?>
                <div class="alert alert-warning" role="alert">
                    ข้อห้อข้อมูลข่าวสารนี้มีแล้ว!
                </div>
            <?php
                ...
                if ($error === 0) {
                    if ($img_size > 125000) {
                        ...
                        if (in_array($img_ex_lc, $allowed_exs)) {
                            ...
                            $sql = "INSERT INTO db_info(info_header, info_detail, info_pic) 
                                    VALUES ('$header', '$detail', '$new_img_name')";
                            if (mysqli_query($conn, $sql)) {
                                $alert = '<script type="text/javascript">';
                                $alert .= 'alert("เพิ่มข้อมูลข่าวสารสำเร็จ!");';
...
<div class="row justify-content-between">
    ...
    <div class="col-auto">
         <a href="?page=<?=$_GET['page']?>" class="btn app-btn-secondary">Back</a>
    </div>
</div>
<hr class="mb-4">
            ...
            <div class="app-card-body">
                <form action="" method="post" enctype='multipart/form-data'>
                    ...
                    <div class="form-floating mb-3 col-lg-3">
                        ...
                        value="<?=(isset($_POST['info_header']) && !empty($_POST['info_header']) ? $_POST['info_header'] : '' )?>" autocomplete="off" required>
